...
export default function AsciiBackground() {
  ...
  const draw = useCallback((time: number) => {
    ...
    const color = glowRaw ? `hsl(${glowRaw} / 0.16)` : "rgba(180,170,150,0.14)";
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const v2 = Math.sin((col + row) * 0.08 + t * 0.5) * 0.42;
        const v3 = Math.cos(col * 0.05 - row * 0.06 + t * 0.3) * 0.24;
        const v = (v1 + v2 + v3 + 1.8) / 3.6; // normalize to 0..1

        if (v > 0.54) {
          ctx.globalAlpha = Math.min(0.8, (v - 0.54) * 2.8);
          ctx.fillText(HEART_GLYPH, x, y);
        }
      }
    }
