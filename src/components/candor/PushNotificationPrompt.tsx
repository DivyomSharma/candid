...
export function PushNotificationPrompt() {
  ...
  useEffect(() => {
    ...
    if (supported && Notification.permission === "default") {
      const hasDismissed = localStorage.getItem("candor-push-dismissed");
      if (!hasDismissed) {
  ...
  const handleDismiss = () => {
    localStorage.setItem("candor-push-dismissed", "true");
    setIsOpen(false);
  };
      ...
      <DrawerContent className="bg-background/80 backdrop-blur-3xl border-t border-border/40 outline-none pb-8">
        ...
        <DrawerHeader className="text-center sm:text-center px-6">
          ...
          <DrawerDescription className="text-base text-foreground-secondary/70 font-light mx-auto max-w-[280px] leading-relaxed">
            allow notifications for new signals, aligns, and when candor has a thought for you.
          </DrawerDescription>
        </DrawerHeader>
