...
export function MemoryControls() {
  ...
  const runAction = async (body: Record<string, unknown>) => {
    setIsBusy(true);
    const response = await fetch("/api/candid/me/memory", {
      method: "PATCH",
      ...
      <CardContent className="grid gap-6 p-5">
        <div>
          ...
          <p className="mt-2 text-sm font-light leading-6 text-foreground-secondary">
            candid keeps understanding, not a permanent transcript. you can remove pieces whenever they feel wrong or too much.
          </p>
        </div>
