...
import {
  ...
  TONIGHTS_THREADS,
  WEEKLY_REFLECTIONS,
} from "@/lib/candid/relational-layer";
import type { CandidContinuity } from "@/lib/candid/continuity";
...
export function RelationalContinuityPanel({ isSignedIn }: RelationalContinuityPanelProps) {
  ...
  const [continuity, setContinuity] = useState<CandidContinuity | null>(null);
  ...
  useEffect(() => {
    ...
    if (isSignedIn) {
      fetch("/api/candid/me/continuity")
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeModule === "small_signal" && isSignedIn && (
          <motion.div
            key="small_signal"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
