...
export function CandidDmChat({ id }: { id: string }) {
  ...
  const sparks = [
    "candid notices they're into obscure cinema... ask about a24?",
    "candid noticed they prefer directness... ask a question they're avoiding?",
    "candid sees they read late... ask about their 2am thoughts?",
    "candid feels the pacing shifted... ask what they're distracted by?",
    "candid notices they like slow openers... ask what they secretly hate?",
  ];
  ...
  useEffect(() => {
    ...
    fetch(`/api/candid/aligns/${id}`)
      ...
      .then((payload: AlignProfile | null) => {
        setAlign(payload);
        if (payload && !payload.canText) router.replace(`/candid/aligns/${id}`);
      });
  ...
  useEffect(() => {
    ...
    const load = () => {
      fetch(`/api/candid/aligns/${id}/messages`, { cache: "no-store" })
        .then((response) => (response.ok ? response.json() : null))
        .then((payload: { messages?: DmMessage[]; locked?: boolean } | null) => {
          if (payload?.locked) {
            router.replace(`/candid/aligns/${id}`);
            return;
          }
          setMessages((current) => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const response = await fetch(`/api/candid/aligns/${id}/messages`, {
      method: "POST",
  ...
  const inviteCandid = async () => {
    if (!align || align.candidInvited || isInviting) return;
    setIsInviting(true);
    const response = await fetch(`/api/candid/aligns/${id}/invite`, { method: "POST" });
    if (response.ok) {
      setAlign((prev) => prev ? { ...prev, candidInvited: true } : null);
    }
    setIsInviting(false);
  };
    ...
    return (
      <main className="gradient-bg grain relative flex min-h-dvh items-center justify-center overflow-hidden px-6">
        ...
        <div className="relative z-10 mx-auto flex max-w-[420px] flex-col items-center gap-6 text-center">
          ...
          <Button onClick={() => router.push(`/candid/login?next=${encodeURIComponent(`/candid/aligns/${id}/chat`)}`)} className="rounded-full bg-accent px-6 text-primary-foreground hover:bg-accent/90">
            sign in
          </Button>
        </div>
      </main>
      ...
      <section className="relative z-10 mx-auto flex max-w-[600px] flex-col gap-8">
        <div className="sticky top-0 z-20 -mx-6 flex items-center justify-between bg-background/45 px-6 py-3 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => router.push(`/candid/aligns/${id}`)} className="rounded-full border border-border/50 p-2 text-foreground-secondary transition-colors hover:text-foreground">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            onClick={inviteCandid}
            disabled={align?.candidInvited || isInviting}
            className={align?.candidInvited 
          ...
          >
            ...
            {align?.candidInvited ? "candid is reading" : isInviting ? "inviting..." : "invite candid"}
          </Button>
        </div>
        ...
        >
          ...
          {messages.map((message) => (
            ...
            >
              ...
              >
                {message.mine ? message.content.replace(/^\[System:[\s\S]*?\]\n\n/, "") : message.content}
              </p>
            </motion.div>
          ))}
