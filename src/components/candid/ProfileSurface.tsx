...
import type { CandidProfilePresentation } from "@/lib/candid/profile";
import type { CandidMemory, CandidBadge } from "@/lib/candid/types";
import { AmbientGlyph } from "@/components/candid/art/AmbientGlyph";
import { WeatherWidget } from "@/components/candid/widgets/WeatherWidget";
import { ZodiacWidget } from "@/components/candid/widgets/ZodiacWidget";
import { SpinningVinyl } from "@/components/candid/widgets/SpinningVinyl";
import { ShelfItemCard } from "@/components/candid/widgets/ShelfItemCard";
...
export function ProfileSurface({
  ...
  profile: CandidProfilePresentation;
  heading: string;
  subheading: string;
  ...
  const [viewerTraits, setViewerTraits] = useState<CandidMemory | null>(null);
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: `${profile.username} on Candid`,
  ...
  const visibleBadges = useMemo(() => {
    if (!v4.badges) return [];
    return v4.badges
      .filter((b: CandidBadge | string) => {
        const confidence = typeof b === "string" ? 0.95 : (b.confidence ?? 0.5);
        return confidence > 0.90;
      })
      .map((b: CandidBadge | string) => typeof b === "string" ? b : b.label)
      .slice(0, 3);
  }, [v4.badges]);
  ...
  const observation = profile.whatCandidNotices?.[0] || "not enough interactions yet.";
  ...
  return (
    <main className="gradient-bg grain relative min-h-dvh overflow-x-hidden px-4 pb-40 pt-16 sm:px-6 sm:pt-20">
      ...
      <section className="relative z-10 mx-auto flex max-w-[900px] flex-col gap-8">
              ...
              <div className="flex shrink-0 flex-wrap items-center justify-center gap-3 md:absolute md:bottom-8 md:right-8 md:justify-end lg:right-10">
                {!publicMode ? (
                  <>
                    <Button
                      type="button"
                      onClick={onEditClick || (() => router.push("/candid/session/ongoing?mode=improve"))}
        ...
        {isProfileLocked && !publicMode && (
          ...
          >
            <div className="relative mb-8">
              <span className="absolute inset-0 rounded-full bg-accent/20 blur-xl animate-[candid-breathe_2.8s_ease-in-out_infinite]" style={{ width: 48, height: 48, margin: 'auto' }} />
            ...
            <p className="text-sm font-light text-foreground-secondary/70 max-w-sm leading-relaxed mb-8">
              talk to candid. answer signals. the more it understands, the more this space reveals.
            </p>
            <Button
              type="button"
              onClick={() => router.push("/candid/session/ongoing")}
        ...
        {!isProfileLocked && (<>
        <div className="candid-desktop-wall auto-rows-min">
            ...
            <Card className="candid-wall-card md:col-span-8 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl flex-1 flex flex-col justify-between shadow-xl">
            ...
            {v4.tonight && v4.tonight.length > 0 && (
              <Card className="candid-wall-card md:col-span-4 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl shadow-xl">
                <CardHeader className="p-5 pb-2">
            ...
            {profile.recentSignals && profile.recentSignals.length > 0 && (
              <Card className="candid-wall-card md:col-span-6 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl shadow-xl">
                <CardHeader className="p-5 pb-2">
                  <CardTitle className="text-xs font-light uppercase tracking-[0.2em] text-foreground-secondary/70">
                    recent signals
                  </CardTitle>
            ...
            {v4.shelf && v4.shelf.length > 0 && (
              <Card className="candid-wall-card md:col-span-6 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl flex-1 flex flex-col justify-between shadow-xl">
                <CardHeader className="p-5 pb-2">
                  <CardTitle className="text-xs font-light uppercase tracking-[0.2em] text-foreground-secondary/70">
                    on my shelf
                  </CardTitle>
            ...
            {v4.openLoops && (v4.openLoops.thinkingAbout || v4.openLoops.recommending || v4.openLoops.defending) && (
              <Card className="candid-wall-card md:col-span-7 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl shadow-xl">
                <CardHeader className="p-5 pb-2">
                  <CardTitle className="text-xs font-light uppercase tracking-[0.2em] text-foreground-secondary/70">
                    open loops
                  </CardTitle>
            ...
            {profile.lookingFor && profile.lookingFor.length > 0 && (
              <Card className="candid-wall-card md:col-span-5 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl shadow-xl">
                <CardHeader className="p-5 pb-2">
                  <CardTitle className="text-xs font-light uppercase tracking-[0.2em] text-foreground-secondary/70">
                    looking for
                  </CardTitle>
            ...
            {v4.smallThings && v4.smallThings.length > 0 && (
              <Card className="candid-wall-card md:col-span-12 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl shadow-xl">
                <CardHeader className="p-5 pb-2">
                  <CardTitle className="text-xs font-light uppercase tracking-[0.2em] text-foreground-secondary/70">
                    small things
                  </CardTitle>
            ...
            <Card className="candid-wall-card md:col-span-6 glass-card border-border/30 bg-card/20 max-md:backdrop-blur-md md:backdrop-blur-3xl border-l-2 border-l-accent flex-1 flex flex-col justify-between shadow-xl">
              <CardHeader className="p-5 pb-2">
              ...
              <CardContent className="p-5 pt-2">
                {!viewerTraits ? (
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Button
                      onClick={() => router.push(`/candid/login?next=${encodeURIComponent(profile.publicPath)}`)}
        ...
        {!publicMode && (
          <motion.div 
            ...
            onClick={() => router.push("/candid/session/ongoing?mode=improve")}
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <p className="text-sm font-light text-foreground-secondary leading-relaxed max-w-xl">
                  your rhythm changes, shelf evolves, and tastes shift. talk to candid to update your profile naturally.
                </p>
              </div>
              <Button 
                type="button"
              ...
              >
                talk to candid <ArrowRight className="h-4 w-4" />
              </Button>
            </div>
          </motion.div>
        )}
