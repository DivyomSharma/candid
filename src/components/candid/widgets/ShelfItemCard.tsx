...
export function ShelfItemCard({ item }: { item: { key: string; value: string } }) {
  ...
  useEffect(() => {
    ...
    if (type) {
      fetch(`/api/candid/search/media?q=${encodeURIComponent(item.value)}&type=${type}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleClick = () => {
    router.push(`/candid/session/ongoing?q=Why+did+you+suggest+the+${encodeURIComponent(item.key)}+"${encodeURIComponent(item.value)}"?`);
  };
