...
import { type CandidPersonalProfile, emptyCandidPersonalProfile } from "@/lib/candid/personal-profile";

type ChatMessage = {
  id: string;
  role: "candid" | "user";
  text: string;
  field?: keyof CandidPersonalProfile | "shelf_book" | "shelf_film" | "cover_url";
};
...
export function PersonalProfileEditor({ profile, profileV4, onSaved }: { profile: CandidPersonalProfile | null; profileV4: Record<string, unknown> | null; onSaved: (p: CandidPersonalProfile, p4: Record<string, unknown> | null) => void }) {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [inputValue, setInputValue] = useState("");
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [draft, setDraft] = useState<CandidPersonalProfile>(profile || emptyCandidPersonalProfile);
  ...
  const nextQuestion = (currentMsgs: ChatMessage[], currentDraft: CandidPersonalProfile) => {
  ...
  const handleSave = async (finalDraft: CandidPersonalProfile) => {
    await fetch("/api/candid/me/profile", {
      method: "PUT",
  ...
  return (
    <div className="flex h-[80vh] flex-col bg-background">
      <div className="flex items-center gap-2 border-b border-white/5 p-4">
        ...
        <h3 className="text-sm font-light text-foreground-secondary">Improve with Candid</h3>
      </div>
