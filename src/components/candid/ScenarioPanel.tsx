...
import type { CandidScenario } from "@/lib/candid/scenarios";
...
function CardContentInner({ 
  ...
  scenario: CandidScenario; 
  onSelect: (scenario: CandidScenario, option: string) => void;
  onShuffle: () => void;
}) {
  let Icon = Sparkles;
...
export function ScenarioPanel({ isSignedIn, onScenarioSelect, onScenarioPrefill }: ScenarioPanelProps) {
  const [scenarios, setScenarios] = useState<CandidScenario[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/candid/me/scenarios")
      .then((res) => (res.ok ? res.json() : null))
      .then((data: { scenarios?: CandidScenario[] } | null) => {
  ...
  const handleSelect = (scenario: CandidScenario, option: string) => {
