...
import { MemoryCard } from "@/components/candid/cards/MemoryCard";
import { OpenLoopCard } from "@/components/candid/cards/OpenLoopCard";
import { CommunityAtmosphereCard } from "@/components/candid/cards/CommunityAtmosphereCard";
import { TruthCard } from "./cards/TruthCard";
import { AmbientGlyph } from "@/components/candid/art/AmbientGlyph";
import { PWAInstallPrompt } from "@/components/candid/PWAInstallPrompt";
import { PushNotificationPrompt } from "@/components/candid/PushNotificationPrompt";
import { SuggestedVacation } from "@/components/candid/widgets/SuggestedVacation";
import dynamic from "next/dynamic";

const SoundtrackCard = dynamic(() => import("@/components/candid/cards/SoundtrackCard").then(mod => mod.SoundtrackCard));
const MovieCard = dynamic(() => import("@/components/candid/cards/MovieCard").then(mod => mod.MovieCard));
const VisualMemoryCard = dynamic(() => import("@/components/candid/cards/VisualMemoryCard").then(mod => mod.VisualMemoryCard));
const MoodCollageCard = dynamic(() => import("@/components/candid/cards/MoodCollageCard").then(mod => mod.MoodCollageCard));
const RandomObjectCard = dynamic(() => import("@/components/candid/cards/RandomObjectCard").then(mod => mod.RandomObjectCard));
const EnvironmentCard = dynamic(() => import("@/components/candid/cards/EnvironmentCard").then(mod => mod.EnvironmentCard));
const ReadingCard = dynamic(() => import("@/components/candid/cards/ReadingCard").then(mod => mod.ReadingCard));

const MoonArt = dynamic(() => import("@/components/candid/art").then(mod => mod.MoonArt));
const ProjectorArt = dynamic(() => import("@/components/candid/art").then(mod => mod.ProjectorArt));
const CoffeeArt = dynamic(() => import("@/components/candid/art").then(mod => mod.CoffeeArt));
const PlantArt = dynamic(() => import("@/components/candid/art").then(mod => mod.PlantArt));
const VinylArt = dynamic(() => import("@/components/candid/art").then(mod => mod.VinylArt));
const CloudArt = dynamic(() => import("@/components/candid/art").then(mod => mod.CloudArt));
const BookOpenArt = dynamic(() => import("@/components/candid/art").then(mod => mod.BookOpenArt));
...
export function CandidHome() {
  ...
  const [signal, setSignal] = useState<CandidSignal | null>(null);
  ...
  const [memoryPreview, setMemoryPreview] = useState<CandidMemory | null>(null);
  ...
  const fetchSignal = useCallback(async (excludeId?: string) => {
    ...
    try {
      const url = excludeId ? `/api/candid/signals?limit=1&excludeId=${excludeId}` : "/api/candid/signals?limit=1";
  ...
  const fetchAligns = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/candid/aligns");
      if (!res.ok) return;
      const data = await res.json();
  ...
  const fetchMemoryPreview = useCallback(async () => {
    if (!isSignedIn) return;
    try {
      const res = await fetch("/api/candid/me/traits");
      if (!res.ok) return;
      const data = await res.json();
  ...
  const fetchReflection = useCallback(async () => {
    if (!isSignedIn) return;
    try {
      const res = await fetch("/api/candid/me/continuity");
      if (!res.ok) return;
      const data = await res.json();
  ...
  const start = useCallback(async (content: string) => {
    ...
    if (!isSignedIn) {
      router.push(`/candid/login?next=${encodeURIComponent("/candid/home")}`);
      return;
    }
    ...
    let success = false;
    try {
      const response = await fetch("/api/candid/conversations", {
        method: "POST",
  ...
  const handleSignalAnswer = useCallback(async (option: string) => {
    ...
    if (!isSignedIn) return;
    try {
      await fetch("/api/candid/signals", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
              ...
              <div className="flex flex-col md:block md:columns-2 lg:columns-3 xl:columns-4 gap-6">
                {masonryCards.map((spec, i) => {
                  const cardEl = renderHomeCard({ 
                    card: { ...spec, priority: 1 } as CandidHomeCardSpec & { artType?: string }, 
                    isSignedIn, preview, previewTeaser, signal, signalAnswered, primaryAlign, adaptiveHome, memoryPreview, reflection, tonightItems, soundtrackUrl, router, fetchSignal, handleSignalAnswer, selectPrompt 
                  });
              ...
              <div className="absolute right-2 flex items-center">
                {isLoaded && isSignedIn ? (
                  ...
                  <button
                    type="button"
                    disabled={!isLoaded}
                    aria-label="Sign in to Candid"
                    onClick={() => router.push(`/candid/login?next=${encodeURIComponent("/candid/home")}`)}
...
function renderHomeCard(input: {
  ...
  if (card.kind === "art") {
    const artType = (card as CandidHomeCardSpec & { artType?: string }).artType || "coffee";
  ...
  if (card.kind === "align") {
    ...
    return (
      <AlignCard 
        ...
        onClick={() => router.push(`/candid/aligns/${primaryAlign.id}`)}
      />
    );
  }
...
function buildYouTonight(memory: CandidMemory | null) {
