...
export function createDefaultSocialState(): CandidSocialState {
  return {
    archetypeSignals: [],
    humorTolerance: 0.5,
...
export function normalizeSocialState(value: unknown): CandidSocialState {
  const input = value as Partial<CandidSocialState> | undefined;
...
export function updateSocialState(input: {
  current?: CandidSocialState;
  message: string;
  memory: CandidMemory;
  move: CandidSocialMove;
...
export function understandingLine(state: CandidSocialState) {
  switch (state.trustStage) {
    case "spark":
      return "candid is beginning to understand your rhythm";
    case "rhythm":
      return "your pace is starting to feel familiar";
    case "patterns":
      return "patterns are becoming clearer lately";
    case "nuance":
      return "candid notices your energy more naturally now";
    case "continuity":
      return "your continuity feels stronger";
    case "resonance":
      return "candid is starting to read the chemistry more deeply";
  }
}

function normalizeTrustStage(value: unknown): CandidSocialState["trustStage"] {
  switch (value) {
    case "glimpse":
    case "warming":
    case "spark":
...
function deriveTrustStage(memory: CandidMemory): CandidSocialState["trustStage"] {
...
function deriveEnergy(text: string, words: number): CandidSocialState["recentEnergy"] {
...
): CandidSocialState["depthAppetite"] {
...
function cleanMoves(value: unknown): CandidSocialMove[] {
  if (!Array.isArray(value)) return [];
  const allowed = new Set<CandidSocialMove>([
    "react",
    "tease",
    "challenge",
    "ask_side_pick",
  ...
  return value.filter((item): item is CandidSocialMove => allowed.has(item)).slice(-8);
}
