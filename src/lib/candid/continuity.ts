...
import { CHEMISTRY_GAMES, TONIGHTS_THREADS, WEEKLY_REFLECTIONS } from "@/lib/candid/relational-layer";
import type { CandidMemory } from "@/lib/candid/types";

export type CandidContinuity = {
...
export async function generateCandidContinuity(memory: CandidMemory): Promise<CandidContinuity> {
  ...
  try {
    const generated = await sendCandidJson<Partial<CandidContinuity>>({
    ...
    console.error("Candid continuity generation failed:", error);
    return fallbackContinuity();
  }
}

function buildContinuityPrompt(memory: CandidMemory) {
  return `
you are candid.
...
function normalizeContinuity(input: Partial<CandidContinuity>): CandidContinuity {
...
function fallbackContinuity(): CandidContinuity {
  const date = new Date();
  return {
