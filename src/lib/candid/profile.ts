import { createEmptyProfileV4, interestLevelMap, topInterestTopics, createEmptyMemory } from "@/lib/candid/memory";
import { ageFromDob, type CandidPersonalProfile } from "@/lib/candid/personal-profile";
import type { CandidMemory, CandidProfileV4 } from "@/lib/candid/types";

export type CandidProfilePresentation = {
  username: string;
  handle: string;
  initials: string;
  publicPath: string;
...
export function buildCandidProfilePresentation(input: {
  memory: CandidMemory | null;
  username?: string | null;
  handle?: string | null;
  email?: string | null;
  personalProfile?: CandidPersonalProfile | null;
}) : CandidProfilePresentation {
  ...
  const bio = personalProfile?.shortBio || (memory && memory.turnCount >= 3
    ? `${username} has been getting to know candid.`
    : "");
  ...
  return {
    ...
    aFewThingsAboutMe,
    lookingFor,
    whatCandidHasLearned,
    recentSignals,
    alignmentAndDepth,
    openDoorStatus,
    coreIdentity,
    whatCandidNotices: notices,
...
function buildUnderstandingDepth(memory: CandidMemory | null): { phase: "spark" | "rhythm" | "patterns" | "nuance" | "continuity" | "resonance"; line: string } {
  ...
  if (turns >= 12) return { phase: "continuity", line: "candid is carrying more of the thread now" };
  ...
  if (turns >= 2 || signalCount >= 2) return { phase: "rhythm", line: "candid is beginning to understand your rhythm" };
  ...
  return {
    ...
    note:
      input.turns >= 3
        ...
        : (input.turns === 0 ? "candid hasn't learned anything yet. start a conversation." : "not enough interactions yet. the rest gets learned in conversation."),
  };
}
...
function buildNotices(memory: CandidMemory | null, values: string[], needs: string[], interests: string[]) {
  const pool = dedupe([
...
function buildResonance(memory: CandidMemory | null, socialPreferences: string[], interests: string[]) {
  const pool = dedupe([
...
function derivedSocialPreferences(memory: CandidMemory | null) {
  const pool = dedupe([
...
function derivedLifestylePreferences(memory: CandidMemory | null, interests: string[]) {
  const pool = dedupe([
