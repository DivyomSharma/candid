import { buildTraitCluster, recordInteractionSignals, topInterestTopics } from "@/lib/candid/memory";
...
export async function getLearningBias(memory: CandidMemory): Promise<CandidLearningBias> {
...
export async function logLearningEvent(userId: string, memory: CandidMemory, event: CandidLearningEvent) {
  try {
    const supabaseAdmin = getSupabaseAdmin();
    ...
    console.error("Candid learning log skipped:", error);
  }
}

export function applyLearningEvent(memory: CandidMemory, event: CandidLearningEvent) {
...
function biasFromMemory(memory: CandidMemory): CandidLearningBias {
...
function structureBiasFromMemory(memory: CandidMemory): CandidStructure[] {
