...
export function createEmptyMemory(): CandidMemory {
  return {
    turnCount: 0,
    lastModes: [],
    values: [],
...
export function normalizeMemory(value: unknown): CandidMemory {
  if (!value || typeof value !== "object") return createEmptyMemory();

  const input = value as Partial<CandidMemory>;
...
export function mergeMemory(existing: CandidMemory, incoming: Partial<CandidMemory>): CandidMemory {
  const merged: CandidMemory = {
    ...existing,
...
export function extractLightMemory(message: string): Partial<CandidMemory> {
  const text = message.toLowerCase();
  const memory: Partial<CandidMemory> = {};
...
export function buildTraitCluster(memory: CandidMemory) {
  const parts = [
    memory.values[0],
    memory.lifeThemes[0],
...
function createEmptyInteractionProfile(): CandidInteractionProfile {
  return {
    choicePatterns: [],
    acceptedInsightTypes: [],
    rejectedInsightTypes: [],
...
function normalizeInteractionProfile(value: unknown): CandidInteractionProfile {
  const input = value as Partial<CandidInteractionProfile> | undefined;
...
function mergeInteractionProfiles(existing: CandidInteractionProfile, incoming?: Partial<CandidInteractionProfile>) {
  return {
...
export function topInterestTopics(memory: CandidMemory, count = 4) {
...
export function interestLevelMap(memory: CandidMemory) {
...
export function addInterestSignals(memory: CandidMemory, interests: Record<string, number>) {
  return {
    ...memory,
    interactionProfile: {
...
export function calculateBadgeConfidences(memory: CandidMemory): CandidBadge[] {
  ...
  const existingBadgesMap = new Map<string, CandidBadge>();
  if (memory.profileV4?.badges) {
    memory.profileV4.badges.forEach((b: CandidBadge | string) => {
...
export function applyUserCorrections(message: string, memory: CandidMemory): CandidMemory {
...
export function createEmptyProfileV4(): CandidProfileV4 {
  return {
    currently: {
      building: "",
      watching: "",
...
export function normalizeBadge(badge: unknown): CandidBadge | null {
...
function normalizeProfileV4(value: unknown): CandidProfileV4 {
  const empty = createEmptyProfileV4();
  if (!value || typeof value !== "object") return empty;
  const val = value as Partial<CandidProfileV4>;
  ...
  return {
    ...
    badges: Array.isArray(val.badges) ? val.badges.map(normalizeBadge).filter((x): x is CandidBadge => !!x) : empty.badges,
  };
}

function mergeProfileV4(existing: CandidProfileV4, incoming?: Partial<CandidProfileV4>): CandidProfileV4 {
  ...
  if (incoming.badges) {
    merged.badges = Array.isArray(incoming.badges) 
      ? incoming.badges.map(normalizeBadge).filter((x): x is CandidBadge => !!x)
      : existing.badges;
  }
  return merged;
}
