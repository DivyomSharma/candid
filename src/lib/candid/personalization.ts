...
export type CandidHomeCardKind =
  | "continue"
  | "signal"
  | "align"
  | "reflection"
...
export type CandidHomeCardSpec = {
  kind: CandidHomeCardKind;
  size: "small" | "medium" | "large" | "tall" | "wide";
  priority: number;
  spanClass?: string;
};

export type CandidAdaptiveHome = {
  hasSufficientData: boolean;
  cards: CandidHomeCardSpec[];
  heroPrompt: string;
...
export function buildAdaptiveHome(memory: CandidMemory | null, seedInput?: string) {
  const now = new Date();
...
function composeHomeCards(input: {
  ...
  const cards: CandidHomeCardSpec[] = [
...
function adaptSurprise(primaryTopic: string | undefined, secondaryTopic: string | undefined, now: Date) {
  ...
  return { label: "memory", line: "you keep circling back to people who feel calm.", detail: "candid noticed the pattern, quietly." };
}

function deriveOpenLoop(memory: CandidMemory | null) {
...
function topTopicsFromMemory(memory: CandidMemory | null) {
  if (!memory) return [];
