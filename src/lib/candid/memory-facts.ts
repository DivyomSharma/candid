...
export async function upsertMemoryFacts(input: {
  ...
  try {
    ...
    await supabaseAdmin.from("candid_memory_facts").upsert(
      facts.map((fact) => ({
    ...
    logCandidInternal({ event: "memory_fact_upsert_skipped", level: "warn", error });
  }
}

export async function factsAsRetrievedMemory(userId: string, limit = 5): Promise<CandidRetrievedMemory[]> {
  try {
    const supabaseAdmin = getSupabaseAdmin();
