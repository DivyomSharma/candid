...
import { getCandidPersonalProfile, handleFromUsername } from "@/lib/candid/personal-profile";

export type PublicCandidIdentity = {
...
export async function getPublicIdentityForCandidUserId(userId: string): Promise<PublicCandidIdentity> {
  ...
  if (!authId) return { username: null, handle: null, age: null, district: null, city: null, lat: null, lon: null, dob: null, coverUrl: null, identityChips: [], candidBadge: null, objects: [], photos: [], shelfItems: [] };
  return getPublicIdentityFromAuthId(authId);
}

export async function getPublicIdentitiesForCandidUserIds(userIds: string[]) {
  const uniqueIds = [...new Set(userIds)];
  if (uniqueIds.length === 0) return new Map<string, PublicCandidIdentity>();
...
async function getPublicIdentityFromAuthId(authId: string): Promise<PublicCandidIdentity> {
  if (isUuid(authId)) return getSupabaseIdentity(authId);
  if (authId.startsWith("user_")) return getClerkIdentity(authId);
  return { username: null, handle: null, age: null, district: null, city: null, lat: null, lon: null, dob: null, coverUrl: null, identityChips: [], candidBadge: null, objects: [], photos: [], shelfItems: [] };
}

async function getSupabaseIdentity(authId: string): Promise<PublicCandidIdentity> {
  const supabaseAdmin = getSupabaseAdmin();
  const { data, error } = await supabaseAdmin.auth.admin.getUserById(authId);
  if (error || !data.user) return { username: null, handle: null, age: null, district: null, city: null, lat: null, lon: null, dob: null, coverUrl: null, identityChips: [], candidBadge: null, objects: [], photos: [], shelfItems: [] };
...
async function getClerkIdentity(authId: string): Promise<PublicCandidIdentity> {
  try {
...
export function publicIdentityFromProfile(profile: Record<string, unknown>) {
  ...
  return {
    ...
    candidBadge: profile?.candid_badge as unknown,
  ...
  } satisfies PublicCandidIdentity;
}
