...
const GENERIC_PRESETS: CandidPresets = {
  chips: [
...
export async function generateCandidPresets(memory: CandidMemory): Promise<CandidPresets> {
  try {
    const generated = await sendCandidJson<Partial<CandidPresets>>({
    ...
    console.error("Candid preset generation failed:", error);
    return fallbackPresets(memory);
  }
}

function buildPresetPrompt(memory: CandidMemory) {
  return `
you are candid.
...
function normalizePresets(input: Partial<CandidPresets>, memory: CandidMemory): CandidPresets {
...
function fallbackPresets(memory: CandidMemory): CandidPresets {
