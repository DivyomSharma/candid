...
export async function getOwnPublicProfile(authId: string) {
  ...
  const { data: traits } = await supabaseAdmin.from("candid_traits").select("data").eq("user_id", user.id).maybeSingle();
...
export async function getPublicProfileByHandle(handleParam: string) {
  ...
  const supabaseAdmin = getSupabaseAdmin();
  const { data: users } = await supabaseAdmin.from("candid_users").select("id");
  ...
  const { data: traits } = await supabaseAdmin.from("candid_traits").select("data").eq("user_id", userId).maybeSingle();
