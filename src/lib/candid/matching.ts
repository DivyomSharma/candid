...
export type PublicCandidProfile = {
  username: string;
  handle: string;
  age: number | null;
...
export function buildPublicProfile(
  ...
  return {
    ...
    candidBadge: identity?.candidBadge ?? null,
...
export function alignmentScore(a: CandidMemory, b: CandidMemory) {
...
export function alignmentLanguage(memory: CandidMemory, other: CandidMemory) {
...
export function alignmentLanguageWithSignals(memory: CandidMemory, other: CandidMemory, signals: string[]) {
...
export function alignmentObservation(memory: CandidMemory, other: CandidMemory, signals: string[]) {
...
export function alignmentWhy(memory: CandidMemory, other: CandidMemory) {
  const need = memory.communicationNeeds[0] ?? other.appreciatesInPeople[0] ?? "gentle honesty";
...
function titleFrom(memory: CandidMemory) {
...
function aboutFrom(memory: CandidMemory) {
  ...
  return "candid is still learning about them.";
}

function oneLineFrom(memory: CandidMemory) {
  const value = memory.values[0];
  const need = memory.communicationNeeds[0];
...
function storySignalFrom(memory: CandidMemory) {
  const theme = memory.lifeThemes[0];
  const value = memory.values[0];
...
function situationFrom(memory: CandidMemory): PublicCandidProfile["situation"] {
...
function usernameFrom(userId: string, memory: CandidMemory) {
  ...
  return `Candid ${userId.slice(0, 4).toUpperCase()}`;
}
