...
import { resolveCandidRoutePlan } from "@/lib/candid/model-registry";
import { candidProviders } from "@/lib/candid/providers";
import type { CandidProviderResult } from "@/lib/candid/providers";
import type { CandidHistoryMessage, CandidRouteMetadata } from "@/lib/candid/transport";

type OrchestratorPayload = CandidRouteMetadata & {
  message: string;
...
export type CandidOrchestrationResult = CandidProviderResult & {
...
export async function orchestrateCandidText(payload: OrchestratorPayload): Promise<CandidOrchestrationResult> {
  const plan = resolveCandidRoutePlan({
  ...
  for (let index = 0; index < plan.attempts.length; index += 1) {
    ...
    const adapter = candidProviders[attempt.provider];
    ...
    try {
      ...
      const finalResult: CandidOrchestrationResult = {
        ...result,
  ...
  throw new Error(`candid_orchestration_failed:${errors.join(",")}`);
}

export async function orchestrateCandidJson<T>(
  ...
  const loosened = await orchestrateCandidText({
    ...payload,
    jsonMode: false,
  });
