import { clearCanonicalMessages, clearSocialState } from "@/lib/candor/persistence";
...
export async function getMemoryControlSnapshot(userId: string) {
  ...
  const [events, facts, messages, initiatives] = await Promise.all([
    safeCount("candor_memory_events", userId),
    safeCount("candor_memory_facts", userId),
    safeCount("candor_messages", userId),
    safeCount("candor_initiatives", userId),
  ]);
...
export async function deleteMemoryEvent(userId: string, id: string) {
  const supabaseAdmin = getSupabaseAdmin();
  await supabaseAdmin.from("candor_memory_events").delete().eq("user_id", userId).eq("id", id);
}

export async function deleteMemoryFact(userId: string, id: string) {
  const supabaseAdmin = getSupabaseAdmin();
  await supabaseAdmin.from("candor_memory_facts").delete().eq("user_id", userId).eq("id", id);
}

export async function clearRelationalMemory(userId: string) {
  const supabaseAdmin = getSupabaseAdmin();
  await Promise.all([
    supabaseAdmin.from("candor_memory_embeddings").delete().eq("user_id", userId),
    supabaseAdmin.from("candor_memory_events").delete().eq("user_id", userId),
    supabaseAdmin.from("candor_memory_facts").delete().eq("user_id", userId),
    supabaseAdmin.from("candor_interaction_patterns").delete().eq("user_id", userId),
    clearSocialState(userId),
  ]);
}

export async function clearEverythingCandorKnows(userId: string) {
  const supabaseAdmin = getSupabaseAdmin();
  await Promise.all([
