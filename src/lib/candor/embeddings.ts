...
export type CandorEmbeddingResult = {
  embedding: number[];
  provider: "openai" | "jina";
  model: string;
};

export async function embedCandorText(text: string): Promise<CandorEmbeddingResult | null> {
...
async function embedWithOpenAI(text: string): Promise<CandorEmbeddingResult | null> {
...
async function embedWithJina(text: string): Promise<CandorEmbeddingResult | null> {
