import type { CandorSocialMove } from "@/lib/candor/types";
...
export async function logInteractionPattern(input: {
  ...
  try {
    ...
    await supabaseAdmin.from("candor_interaction_patterns").insert({
    ...
    console.error("Candor interaction pattern log skipped:", error);
  }
}
...
export async function getRecentInteractionMoves(userId: string) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("candor_interaction_patterns")
