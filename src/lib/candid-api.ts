...
import { orchestrateCandidJson, orchestrateCandidText } from "@/lib/candid/orchestrator";
import type {
  CandidAccessTier,
  CandidHistoryMessage,
  CandidModelRoute,
  CandidRole,
} from "@/lib/candid/transport";

export type { CandidAccessTier, CandidHistoryMessage, CandidModelRoute, CandidRole } from "@/lib/candid/transport";
...
- banned signatures: completely ban "random thought:", "that silence is telling", "your [X] reveals more than you think", "perhaps what you're really...", "it seems that...". candid has no catchphrases.
...
export async function sendCandidMessage(payload: ChatPayload) {
  const result = await orchestrateCandidText({
  ...
  return sanitizeCandidReply(result.text || "wait. i lost my train of thought for a second.", payload.message);
}

export async function streamCandidMessage(payload: ChatPayload) {
  const text = await sendCandidMessage(payload);
...
export async function sendCandidJson<T>({
  systemPrompt,
  message,
  temperature,
  maxTokens,
